...
export function InvoiceList({ invoices, filters, selectedId, loading, onFiltersChange, onSelect, onCreate, canCreate }: InvoiceListProps) {
  return (
    <section className="panel invoice-list-panel" aria-labelledby="invoice-list-title">
      ...
      <div className="invoice-filters">
        <label className="search-field"><input value={filters.query} placeholder="請求番号・顧客・件名で検索" onChange={(event) => onFiltersChange({ ...filters, query: event.target.value })} /></label>
        <label><span>状態</span><select value={filters.status} onChange={(event) => onFiltersChange({ ...filters, status: event.target.value as InvoiceFilters['status'] })}><option value="">すべて</option><option value="draft">下書き</option><option value="issued">発行済み</option><option value="overdue">期限超過</option><option value="paid">入金済み</option><option value="cancelled">取消済み</option></select></label>
      </div>
        ...
        {invoices.map((invoice) => {
          ...
          return (
            <button key={invoice.id} type="button" className={invoice.id === selectedId ? 'invoice-row selected' : 'invoice-row'} onClick={() => onSelect(invoice)}>
              <span className="invoice-row-main"><strong>{invoice.invoice_number}</strong><small>{invoice.customer_name_snapshot} ／ {invoice.subject || '件名未設定'}</small></span>
