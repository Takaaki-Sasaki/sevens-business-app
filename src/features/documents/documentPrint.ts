...
function formatDiscountLabel(rateBasisPoints: number | null): string {
  if (rateBasisPoints === null) return '割引';
  const percent = (rateBasisPoints / 100).toLocaleString('ja-JP', { maximumFractionDigits: 2 });
  return `割引（${percent}%）`;
}

function formatDiscountYen(value: number): string {
  return value === 0 ? '0' : `−${formatYen(value)}`;
}
...
export function documentMarkup(data: DocumentData): string {
  const rows = Array.from({ length: 9 }, (_, index) => data.lines[index]);
  ...
  const discountLabel = escapeHtml(formatDiscountLabel(data.orderDiscountRateBasisPoints));
...
function documentPaperStyles(unit: 'px' | 'mm'): string {
  if (unit === 'px') return `
    ...
    .doc-meta { position:absolute; top:294px; left:80px; display:grid; gap:11px; width:340px; margin:0; font-size:14px; line-height:1.2; }.doc-meta div { display:grid; grid-template-columns:82px 1fr; }.doc-meta dt,.doc-meta dd { margin:0; }.doc-meta dd { overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }.doc-meta .doc-bank { max-height:2.7em; line-height:1.35; white-space:pre-line; }
  ...
  return `
    ...
    .doc-meta { position:absolute; top:76mm; left:12mm; display:grid; gap:3mm; width:110mm; margin:0; font-size:10.5pt; line-height:1.2; }.doc-meta div { display:grid; grid-template-columns:25mm 1fr; }.doc-meta dt,.doc-meta dd { margin:0; }.doc-meta dd { overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }.doc-meta .doc-bank { max-height:2.7em; line-height:1.35; white-space:pre-line; }
